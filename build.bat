...
if /I "%~1"=="test" set "BUILD_TARGET=photon_tracer ppt_unit_tests ppt_convergence_tests ppt_diagnostics_tests"
...
if /I "%~1"=="editor" set "BUILD_TARGET=scene_editor"
if /I "%~1"=="editor" set "CMAKE_EXTRA=-DPPT_BUILD_EDITOR=ON"
if /I "%~1"=="editor" echo [build.bat] Building scene editor.
shift
goto :parse_args
:args_done
...
if defined NEED_CONFIGURE (
    ...
    REM Clean FetchContent subbuild caches to prevent generator mismatch on reconfigure
    for /d %%D in ("%BUILD_DIR%\_deps\*-subbuild") do (
        if exist "%%D\CMakeCache.txt" del /q "%%D\CMakeCache.txt" 2>nul
