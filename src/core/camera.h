#pragma once
// ─────────────────────────────────────────────────────────────────────
// camera.h – Thin-lens camera model (v5)
//
// Exact port from v4.  Pinhole when lens_radius == 0.
// ─────────────────────────────────────────────────────────────────────
#include "core/types.h"
#include "core/config.h"
#include "core/random.h"

// ── Unified HD camera ray generation ────────────────────────────────
...
struct Camera {
    ...
    float  dof_focus_dist  = DEFAULT_DOF_FOCUS_DISTANCE;
    float  dof_f_number    = DEFAULT_DOF_F_NUMBER;
    float  sensor_height   = DEFAULT_DOF_SENSOR_HEIGHT;
    float  dof_focus_range = DEFAULT_DOF_FOCUS_RANGE;

    // Derived DOF
    float  focal_length = 0.f;
    float  lens_radius  = 0.f;

    // Derived camera frame
    float3 u, v, w;
    float3 lower_left;
    float3 horizontal;
    float3 vertical;
    ...
    void update() {
        ...
        focal_length = sensor_height / (2.0f * h);
