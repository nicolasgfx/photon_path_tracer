...
// is_srgb: true for color textures (map_Kd, map_Ks, map_Ke), false for data (bump, alpha).
static int load_or_reuse_texture(Scene& scene, const std::string& full_path,
    ...
    if (is_srgb && desired_channels >= 3) {
        // Color textures: sRGB → linear (RGB channels only, alpha stays linear)
        for (int i = 0; i < w * h; ++i) {
            int base = i * desired_channels;
            tex.data[base + 0] = srgb_to_linear(img[base + 0] / 255.0f);
            tex.data[base + 1] = srgb_to_linear(img[base + 1] / 255.0f);
            tex.data[base + 2] = srgb_to_linear(img[base + 2] / 255.0f);
            if (desired_channels >= 4)
                tex.data[base + 3] = img[base + 3] / 255.0f;
        }
    } else {
        // Data textures (bump, alpha): keep linear
...
static bool load_mtl(const std::string& filepath, Scene& scene,
    ...
    while (std::getline(file, line)) {
        ...
        if (keyword == "newmtl") {
            ...
            float r, g, b;
            ss >> r >> g >> b;
            scene.materials[current_idx].Kd = Color3::from_rgb(r, g, b);
        }
        else if (keyword == "Ks") {
            float r, g, b;
            ss >> r >> g >> b;
            scene.materials[current_idx].Ks = Color3::from_rgb(r, g, b);
        }
        else if (keyword == "Ke") {
            // Emission
            float r, g, b;
            ...
            if (r > 0.f || g > 0.f || b > 0.f) {
                ...
                scene.materials[current_idx].Le = Color3::from_rgb(r, g, b);
            ...
            float r, g, b;
            ss >> r >> g >> b;
            scene.materials[current_idx].Tf = Color3::from_rgb(r, g, b);
        }
        else if (keyword == "d") {
            float d_val;
            ss >> d_val;
            ...
            int ti = load_or_reuse_texture(scene, full_path, full_path, 4, false);
            ...
            if (ti >= 0) {
                ...
                if (scene.materials[current_idx].Le.max_component() == 0.f) {
                    scene.materials[current_idx].Le = Color3::constant(1.0f);
                    scene.materials[current_idx].type = MaterialType::Emissive;
                }
            }
        }
        ...
        // ── Photon-Beam material extensions (pb_*) ──────────────────
            ...
            auto& m = scene.materials[current_idx];
            float r_val = 1.f, g_val = 1.f, b_val = 1.f;
            ss >> r_val >> g_val >> b_val;
            m.pb_tf_color = Color3::from_rgb(r_val, g_val, b_val);
            m.pb_tf_color_set = true;
        }
...
// Non-static: also called by the scene loader.
void finalize_pb_materials(Scene& scene) {
    ...
    for (auto& mat : scene.materials) {
        ...
        if (mat.pb_conductor_set) {
            mat.pb_conductor_eta_spec = Color3::from_rgb(
                mat.pb_conductor_eta_rgb[0],
                mat.pb_conductor_eta_rgb[1],
                mat.pb_conductor_eta_rgb[2]);
            mat.pb_conductor_k_spec = Color3::from_rgb(
        ...
        // pb_tf_color bypasses the default Tf, allowing direct colour
        // filters (e.g. deep green glass).
        if (mat.pb_tf_color_set) {
            mat.Tf = mat.pb_tf_color;
        }

        // ── 4. Transmission override ────────────────────────────────
        // DiffuseTransmission handles transmission through the BSDF,
        // not stochastic opacity.
        if (mat.pb_transmission_set && mat.type != MaterialType::DiffuseTransmission) {
            mat.opacity = 1.0f - mat.pb_transmission;
        }

        // ── 5. Map pb_brdf → MaterialType ───────────────────────────
        // pb_brdf takes precedence over illum-derived type.
        // DiffuseTransmission is set directly by the scene mapper — skip.
        if (mat.type == MaterialType::DiffuseTransmission) {
            // Already assigned by map_diffuse_transmission(); don't override.
        } else switch (mat.pb_brdf) {
            case PbBrdf::Lambert:
                mat.type = MaterialType::Lambertian;
                break;
            ...
            case PbBrdf::Dielectric: {
                ...
                // Thin sheets are always glass (transparency via specular bounce,
                // not stochastic alpha).  Without this, low-transmittance thin
                // materials (leaves, lampshades) become opaque GlossyDielectric.
                bool is_glass = mat.pb_thin
                             || (mat.pb_transmission_set
            ...
            case PbBrdf::Conductor:
                ...
                if (mat.pb_conductor_set) {
                    ...
                    for (int ch = 0; ch < 3; ++ch) {
                        float n = mat.pb_conductor_eta_spec[ch];
                        float k = mat.pb_conductor_k_spec[ch];
                        float num = (n - 1.f) * (n - 1.f) + k * k;
                        float den = (n + 1.f) * (n + 1.f) + k * k;
                        mat.Ks[ch] = (den > 0.f) ? num / den : 0.5f;
                    }
...
bool load_obj(const std::string& filepath, Scene& scene) {
    ...
    // Add a default material
    {
        ...
        default_mat.Kd   = Color3::constant(0.5f);
    ...
    if (!smooth_tris.empty()) {
        ...
        // Auto-smooth: if a vertex smooth normal deviates too far from
        // the triangle's geometric normal (> ~45°), use the geometric
        // normal instead.  This prevents averaging across sharp edges
        // (e.g. 90° wall junctions in Cornell box) that cause dark
        // shadow-acne artifacts.
        constexpr float AUTO_SMOOTH_COS = 0.707f;  // cos(45°)
        int sharp_verts = 0;
        for (const auto& ref : smooth_tris) {
            Triangle& tri = scene.triangles[ref.tri_idx];
            float3 gn = normalize(tri.geometric_normal());
            auto apply = [&](float3& sn, int vi) {
                if (vi >= 0 && vi < (int)accum.size()) {
                    if (dot(accum[vi], gn) >= AUTO_SMOOTH_COS) {
                        sn = accum[vi];
                    } else {
                        sn = gn;
                        ++sharp_verts;
            ...
            apply(tri.n0, ref.vi0);
            apply(tri.n1, ref.vi1);
            apply(tri.n2, ref.vi2);
        }

        std::cout << "[OBJ] Computed area-weighted smooth normals for "
                  << smooth_tris.size() << " triangles";
        if (sharp_verts > 0)
            std::cout << "  (" << sharp_verts << " verts kept flat at sharp edges)";
        std::cout << "\n";
    }
