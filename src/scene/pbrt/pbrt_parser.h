...
bool         get_float3(const std::vector<Param>& params, const std::string& name,
                        double out[3]);
...
private:
    ...
    std::unordered_map<std::string, Mat4> coordinate_systems_;
    std::vector<std::string> file_stack_;

    std::vector<Param> shape_attributes_;
    std::vector<Param> light_attributes_;
    std::vector<Param> material_attributes_;
    std::vector<Param> texture_attributes_;
    std::vector<Param> medium_attributes_;
    ...
    struct GraphicsState {
        ...
        std::string medium_interior;
        std::string medium_exterior;
        std::vector<Param> shape_attributes;
        std::vector<Param> light_attributes;
        std::vector<Param> material_attributes;
        std::vector<Param> texture_attributes;
        std::vector<Param> medium_attributes;
    };
    ...
    void dispatch(const std::vector<std::string_view>& tokens, const std::string& base_dir);

    // Tokenizer — returns string_views into the source buffer (must stay alive)
    static std::vector<std::string_view> tokenize(const char* text, size_t len);

    // Parameter parser: parses "type name" [values] pairs
    static size_t parse_params(const std::vector<std::string_view>& tokens, size_t pos,
