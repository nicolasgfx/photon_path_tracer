// ─────────────────────────────────────────────────────────────────────
// postfx/bloom.cu – GPU bloom kernels (mip-chain approach, v5)
// ─────────────────────────────────────────────────────────────────────
    ...
    float lo_threshold, float hi_threshold)
{
    ...
    float strength = 0.f;
    constexpr float BLOOM_MIN_STRENGTH = 0.10f;
...
// ── Adaptive bright-extract: reads max_lum from device pointer ──────

__global__ void bloom_bright_extract_adaptive_kernel(
    const float* __restrict__ hdr,
    float*       __restrict__ mip0,
    const float* __restrict__ max_lum_ptr,
    int src_w, int src_h,
    int dst_w, int dst_h,
    float scene_min_Le, float scene_max_Le)
{
    ...
    // Compute thresholds on-device (mirrors host logic in apply_bloom_)
    float lo_threshold, hi_threshold;
    if (scene_min_Le > 0.f && scene_max_Le > 0.f
        && scene_max_Le > scene_min_Le * 1.01f)
    {
        float ratio = scene_min_Le / scene_max_Le;
    ...
    float scene_min_Le, float scene_max_Le)
{
    ...
    dim3 block(16, 16);
    dim3 grid((dst_w + 15) / 16, (dst_h + 15) / 16);
    bloom_bright_extract_adaptive_kernel<<<grid, block>>>(
        d_hdr, d_mip0, d_max_lum, src_w, src_h, dst_w, dst_h,
        scene_min_Le, scene_max_Le);
}
