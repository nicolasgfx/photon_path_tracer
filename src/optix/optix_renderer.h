...
public:
    ...
    /// Build the direction map for the current camera / photon map.
    /// Launches __raygen__direction_map at (dir_map_width × dir_map_height).
    void build_direction_map(const Camera& camera, int spp_seed);

    /// Download direction map from GPU to host (for debug PNG output).
    void download_direction_map();
...
private:
    ...
    OptixProgramGroup        raygen_dirmap_pg_   = nullptr;  // direction map build
    ...
    DeviceBuffer d_raygen_dirmap_record_;
...
inline void OptixRenderer::cleanup() {
    ...
    if (raygen_dirmap_pg_)     { optixProgramGroupDestroy(raygen_dirmap_pg_);   raygen_dirmap_pg_ = nullptr; }
