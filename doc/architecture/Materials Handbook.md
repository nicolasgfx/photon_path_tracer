# Material Extensions for Wavefront MTL (`pb_*`)

**Status:** Implemented — all `pb_*` tokens are parsed and stored; surface BSDFs (9 material types) are fully rendered on the GPU. Per-material interior media (`pb_medium homogeneous`) are parsed and stored; GPU upload infrastructure exists (`mat_medium_id` in LaunchParams) but is currently set to `nullptr` — full GPU medium transport is not yet wired in v5.
...
- **RGB pipeline:** all `Kd`/`Ks`/`Ke`/colour parameters are RGB triples used directly by the renderer (no spectral conversion).
...
- On the GPU, three representative wavelengths (R=630nm, G=532nm, B=465nm) are used in the caustic tracer; each channel refracts at its own Cauchy IOR with independent Fresnel weights.
...
Represents a **complex IOR** per channel (RGB). Renderer uses these to compute per-channel Fresnel for conductors (Schlick approximation: `F0 = ((n-1)²+k²)/((n+1)²+k²)`).
...
| `pb_roughness` | float | perceptual roughness (α = r²) | most non-matte |
...
| `pb_tf_spectrum` | vec3 | direct transmittance override (parsed as RGB, stored as `pb_tf_color`) | glass colour |
...
#### Per-material interior medium (`pb_medium`) — Parsed and stored

The `pb_medium` pipeline is **parsed and stored** but **not fully wired on the GPU in v5**:

1. All tokens below are parsed and stored in the `Material` struct.
2. `finalize_pb_materials()` creates a `HomogeneousMedium` ($\sigma_a$, $\sigma_s$, $\sigma_t$, $g$), applies `pb_density / pb_meters_per_unit` scaling, and stores it in `Scene::media`.
3. The material's `medium_id` links to its entry; `MaterialType::Translucent` is set.
4. **GPU status:** `LaunchParams` has `mat_medium_id` field and `MediumStack` infrastructure exists in `specular.h`, but `mat_medium_id` is currently set to `nullptr` in `accel_builder.cpp`. Translucent materials render as Glass (Fresnel + `Tf` transmission filter) without interior scattering.

**What this means in practice:** adding `pb_medium homogeneous` to a material will:
- correctly mark it as `Translucent` (not `Glass`),
- store the medium coefficients on the CPU,
- but on the GPU it will render as Glass with a `Tf` colour filter (no volumetric scattering).

**Note:** Full GPU medium transport (free-flight, HG phase function, Beer–Lambert) is infrastructure that exists but is not yet connected in v5.

#### Global atmospheric medium — removed in v5

The v4 scene-wide Rayleigh participating medium (controlled via the **V** key and `RenderConfig::volume_*` fields) has been **removed** in v5. The v5 renderer does not support global atmospheric media.

For per-object translucency, use `pb_medium homogeneous` — the tokens are parsed and stored, but GPU rendering is limited to Glass-style `Tf` filtering until full medium transport is wired (see above).
...
Implementation note:
- `Kd/Ks/Ke/pb_sigma_*` RGB triples are used directly as linear RGB by the renderer.
- `map_Kd` textures are assumed sRGB unless specified otherwise.
- Numeric triples in `.mtl` are assumed linear (common in exporters), but this should be validated for your content pipeline.
...
1) **Glass / transmissive dielectric**
  ...
  - **GPU behaviour:** adding `pb_medium` promotes the type to `Translucent` (separate enum). In v5 the GPU renders Translucent as Glass with a `Tf` colour filter; full interior medium transport (Beer–Lambert, free-flight, HG) is not yet connected.
...
> **Note:** Per-material interior media tokens are parsed and stored, but GPU medium transport is not yet wired in v5. Translucent materials render as Glass with a `Tf` colour filter. The recipes below define the intended material properties — full volumetric scattering is pending.
...
| 3 | **Convert colors:** RGB colour properties uploaded to GPU | ✅ Done — `Color3` RGB values uploaded directly (no spectral conversion in v5) |
| 4 | **Pick BRDF:** from `pb_brdf` (or derived) | ✅ Done — all 6 BRDF types mapped to `MaterialType` enum |
| 5 | **Layering:** if `clearcoat`, build layered model | ✅ Done — `pb_clearcoat`, `pb_clearcoat_roughness`, coat IOR uploaded via `d_clearcoat_weight_` / `d_clearcoat_roughness_` device buffers and read by GPU Clearcoat BSDF |
| 6 | **Media:** if `pb_medium homogeneous`, attach interior medium | ⚠️ **Partial** — CPU creates `HomogeneousMedium` & sets `medium_id`; GPU has `mat_medium_id` field but sets it to `nullptr` (Translucent renders as Glass + Tf filter) |
...
> **Note:** The `pb_medium` tokens are parsed and stored. In v5,
> Translucent renders as Glass + `Tf` filter; full volumetric
> scattering (Beer–Lambert + free-flight + HG) is not yet wired on GPU.
...
## 11) PBRT v4 material mapping

PBRT v4 scenes (`.pbrt` files) are the primary input format.  The `pbrt_material_mapper.cpp` maps each PBRT material type to our internal `pb_*` system, which `finalize_pb_materials()` then converts to a `MaterialType` enum for the GPU.

### 11.1 Type mapping table

| PBRT v4 type | → `pb_brdf` | → `MaterialType` | Notes |
|---|---|---|---|
| `diffuse` | Lambert | Lambertian | Texture → `diffuse_tex`; Kd=1.0 when textured |
| `coateddiffuse` | Clearcoat | Clearcoat | Coat GGX + Lambert base; roughness via `pb_clearcoat_roughness`; coat IOR from `eta` param (default 1.5) |
| `conductor` | Conductor | GlossyMetal | Complex IOR → F0 via `((n-1)²+k²)/((n+1)²+k²)`; named spectrum presets supported |
| `coatedconductor` | Conductor | GlossyMetal | **Approximation**: thin coat highlight (~4% F0) is lost; mapped as plain conductor |
| `dielectric` (smooth) | Dielectric | Glass | α ≤ 0.065 → delta Glass with Fresnel refraction |
| `dielectric` (rough) | Dielectric | GlossyDielectric | α > 0.065 → opaque rough plastic; **no microfacet refraction** |
| `thindielectric` | Dielectric | Glass | `pb_thin=true`; straight-through transmission with Fresnel |
| `diffusetransmission` | Dielectric | DiffuseTransmission | Two-sided Lambert: Kd/π reflect + Tf/π transmit |
| `subsurface` | Lambert | Lambertian | **No true SSS** — fallback to diffuse approximation |
| `measured` | Clearcoat | Clearcoat | **Approximation**: clearcoated white diffuse |
| `hair` | Lambert | Lambertian | Color from reflectance, sigma_a, or melanin model |
| `interface` | Dielectric | Glass | Invisible medium boundary (IOR=1.0, opacity=0) |
| `mix` | varies | varies | Picks sub-material closest to `amount=0.5` threshold |

### 11.2 Roughness conversion chain

PBRT v4 roughness parameters → our internal system:

```
PBRT scene: roughness R, remaproughness=true/false
    ↓
pbrt_roughness_to_ours(R, remap):
    if remap:  our_r = R^(1/4)          ← perceptual roughness
    else:      our_r = sqrt(R)          ← R is GGX alpha already
    ↓
stored in: pb_roughness or pb_clearcoat_roughness
    ↓
uploaded to GPU as: roughness[mat_id] or clearcoat_roughness[mat_id]
    ↓
GPU BSDF: alpha = our_r² = bsdf_roughness_to_alpha(our_r)
    ↓
final alpha:
    remap=true:  alpha = (R^(1/4))² = sqrt(R)   ← matches PBRT's RoughnessToAlpha()
    remap=false: alpha = (sqrt(R))² = R          ← direct passthrough
```

### 11.3 Texture convention

When a PBRT material references a texture for `reflectance`, the texture IS the full albedo.  The material mapper sets `Kd = Color3(1.0)` so that the GPU's `dev_get_Kd(mat_id, uv)` — which returns `base_Kd × tex_color` — passes the texture value through unmodified.

For non-textured materials, `Kd` is set to the constant reflectance RGB value from the scene file (default 0.5 grey if unspecified).

### 11.4 Known limitations

| Limitation | Impact | Workaround |
|---|---|---|
| No microfacet refraction | Rough dielectrics render as opaque plastic | Use smooth Glass or accept plastic-like appearance |
| `coatedconductor` → GlossyMetal | Loses thin coat highlight (~4% at normal incidence) | Accept; visual difference is subtle for rough metals |
| Clearcoat: additive blend, not stochastic layers | Slight energy non-conservation at grazing angles | Accept; difference is small for typical coat roughness |
| `measured` → clearcoat approximation | Generic appearance for measured BRDFs | Accept or provide manual material override |
| No true SSS for `subsurface` | SSS materials render as plain diffuse | Use `pb_medium homogeneous` for volumetric scattering |
| `mix` → picks single sub-material | No spatial/parametric blending | Accept; uses sub-material closest to 50% mix |
