...
5. **Caustic deposit:** When the photon finally hits a diffuse surface outside, it is deposited with `PHOTON_FLAG_TRAVERSED_GLASS` set (and possibly `PHOTON_FLAG_DISPERSION`). The photon forms a **caustic** on the surrounding surfaces.
...
| Glass | Yes | No | Fresnel reflect/refract | `TRAVERSED_GLASS` |
...
| Translucent | Yes (surface) | No | Fresnel reflect/refract (+ medium on CPU) | `TRAVERSED_GLASS` |
...
| `PHOTON_FLAG_VOLUME_SCATTER` (0x04) | Any volume scatter event |
