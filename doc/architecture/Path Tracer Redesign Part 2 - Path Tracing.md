...
**Use:** Cells with a high fraction of caustic photons (`TRAVERSED_GLASS`, `CAUSTIC_SPECULAR`) carry high-frequency light transport that the path tracer cannot efficiently reproduce.
...
| `PHOTON_FLAG_VOLUME_SEGMENT` | Removed (legacy alias for `PHOTON_FLAG_VOLUME_SCATTER`) |
| `PHOTON_FLAG_CAUSTIC_GLASS` | Removed (redundant with `TRAVERSED_GLASS`) |
